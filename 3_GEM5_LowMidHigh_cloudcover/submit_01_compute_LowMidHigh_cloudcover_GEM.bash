...
###########################################################################
if [[ $layerdef != pmax ]] && [[ $layerdef != pmin ]]&& [[ $layerdef != pran ]] && [[ $layerdef != pmean ]]  && [[ $layerdef != amax ]] && [[ $layerdef  != amin ]] && [[ $layerdef != amean ]]; then
        echo "ERROR: =$layerdef"
...
#                            Script submission                            #
###########################################################################
script=$working_directory/wrapper_01_compute_LowMidHigh_cloudcover_GEM.bash
...
while [ $YYYYMM -le $YYYYMMf ]; do
	...
	if [[ $submission_type == scheduler ]]; then
		soumet $script -args $args -jn GEM_LowMidHigh_cloudcover_$YYYYMM #-t 111600
