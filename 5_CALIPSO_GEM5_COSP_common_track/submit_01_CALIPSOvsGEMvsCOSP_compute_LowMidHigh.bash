...
if [ $# -ne 5 ]; then
	echo "USAGE:   ./submit_01_CALIPSOvsGEMvsCOSP_compute_LowMidHigh.bash YYYYMMi YYYYMMf layerdef  overwrite submission_type"
	echo "EXAMPLE: ./submit_01_CALIPSOvsGEMvsCOSP_compute_LowMidHigh.bash 201401  201512  pmax      false     scheduler"
	echo "EXAMPLE: ./submit_01_CALIPSOvsGEMvsCOSP_compute_LowMidHigh.bash 201401  201402  amin      false     interactive"
	echo ""
    ...
    echo "layerdef: ran  --> Random cloud cover  (of all relevant layers"
...
###########################################################################
if [[ $layerdef != pmax ]] && [[ $layerdef != pmaxran ]] && [[ $layerdef != pmin ]]&& [[ $layerdef != pran ]] && [[ $layerdef != pmean ]]  && [[ $layerdef != amax ]] && [[ $layerdef  != amin ]] && [[ $layerdef != amean ]]; then
        echo "ERROR: =$layerdef"
        echo "ERROR: Accepted values [pmax/pmin/pmean/amax/amin/amean]"
	echo "ERROR: pressure(p) --> L 680hpa M 440hpa H"
        ...
        echo "ERROR: ran  --> random cloud cover  (of all relevant layers)"
        echo "ERROR: max-ran --> max-ran cloud cover (of all relevant layers)"
...
#                            Script submission                            #
###########################################################################
script=$working_directory/wrapper_01_CALIPSOvsGEMvsCOSP_compute_LowMidHigh.bash
