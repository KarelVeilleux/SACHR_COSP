...
# Remove the entry corresponding to the same date (cloudsat track split in a day file and a night file)
...
if overwrite == 0:
    ...
    for i in range(len(df['date_gem'])):
        ...
        if os.path.isfile(file):
            ...
            stream           = os.popen('ncinfo ' + file + '&& echo 1 || echo 0') 
            #print('xxxx',stream.read()[-2])
            file_is_readable = int(stream.read()[-2])
