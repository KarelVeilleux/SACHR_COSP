...
YYYYMM=201401
input_list_file = '/pampa/poitras/DATA/CAL_LID_L2_05kmCPro-Standard-V4/list/hudson_bay_11_2014.txt'
df              = pd.read_csv(input_list_file,delimiter='\s+', header=None)
df.columns      = ['nc_CALIPSO', 'nadata', 'ti', 'tf', 'date', 'date_gem', 't_gem']
df             = df[   df['date'] >= 2014010100].reset_index(drop=True)
df             = df[   df['date'] <= 2014013123].reset_index(drop=True)
print(df)
...
for i in range(len(df['date_gem'])):
    ...
    ncfile_cospin  = dircosp + '/' + MP + '/INPUT/'  + 'RESIZED/cosp_input_'  + date + '.nc'
    ncfile_cospout = dircosp + '/' + MP + '/OUTPUT/' +  MSC + '/cosp_output_' + date + '_calipso_cloudprofile_2D.nc'
...
# INTENSITY-FREQUENCY FIGURES
    ...
    for threshold in [0,1e-6]:
        ...
        img_att = {'title':'Total cloud cover ' , 'vstruct':'calipso', 'fignamex': dirfig + '/freq_int/' + 'cloud_cover_total_xxxYYY_' + th + '_' + date + '_calipso_COSP2_GEM5'   }
