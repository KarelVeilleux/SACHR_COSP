...
input_list_file = '/pampa/poitras/DATA/CAL_LID_L2_05kmCPro-Standard-V4/list/' + domain + '_2014.txt'
df              = pd.read_csv(input_list_file,delimiter='\s+', header=None)
df.columns      = ['nc_CALIPSO', 'ndata', 'ti', 'tf', 'date', 'date_gem', 't_gem']
...
for i in range(len(df['date_gem'])):
    file='/pampa/poitras/DATA/COSP2/Cascades_CORDEX/CLASS/COSP2_NAM-11m_ERA5_GEM5_CLASS_NEWVEG_newP3-SCPF_SN_Lakes/MPB/INPUT/' + domain + '/2014/cosp_input_'  + str(df['date'][i]) + '.nc'
...
dirgem      = '/pampa/poitras/DATA/TREATED/GEM5/COSP2/' + gempath + '/Samples_NetCDF'
...
for i in range(len(df['date_gem'])):
    ...
    ncfile_cospin  = dircosp + '/' + MP + '/INPUT/'  + domain + '/2014/cosp_input_'  + date + '.nc'
    ncfile_cospout = dircosp + '/' + MP + '/OUTPUT/' + domain + '/' + MSC + '/cosp_output_' + date + '_calipso_cloudprofile_2D.nc'
...
for threshold in [0,0.03333333333]:
    ...
    img_att = {'masktype':masktype , 'vstruct':'calipso', 'figname': dirfig + '/freq_int/' + 'cloud_cover_total_xxxYYY_' +  date  + '_' + maskfname + '_' + s +  '_calipso_COSP2_GEM5_' + th }
...
image_attribute_map = {'figname': dirfig + '/../track/cloudsat_' +  str(df['ti'][i]) + '_' + str(df['tf'][i])}
